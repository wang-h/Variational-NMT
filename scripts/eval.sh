...
cp ${OUTPUT}/eval/test.pred.5 ${OUTPUT}/eval/test.ja
...
for file in test; do
 ...
 spm_decode --model /itigo/Uploads/ASPEC-JC.clean/bpe_char/bpe_char.ja.model < ${file}.ja | sed -r 's/(@@ )|(@@ ?$)//g' | \
...
#cat test.${TASK}.juman.ja | cut -c 4- > test.${TASK}.juman.ja1
#rm test.${TASK}.juman.ja
#cat test.${TASK}.juman.ja1 | sed 's/.\(.*\)/\1/' >  test.${TASK}.juman.ja
# For BLEU
